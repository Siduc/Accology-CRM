...
class CsPack(Base):
    ...
    # XML Gateway CS01 export / submit (Software Filing)
    xml_export_json = Column(Text, nullable=True)
    xml_last_export_at = Column(DateTime, nullable=True)
    xml_submission_ref = Column(String, nullable=True)
    xml_submission_status = Column(String, nullable=True)
    xml_submission_response = Column(Text, nullable=True)
